...
def perform_op(op:str, from_alignms:dict, to_alignms:dict):
...
def align_spans(gld_alignms:dict, prd_alignms:dict, op=['delO','no-choice','intrsct']) -> dict:
    ...
    if len([span for span in gld_alignms if len(gld_alignms[span]) > 1]) == 0:
        ...
        gld_aligned = False
        gld_alignms, prd_alignms = perform_op(op=op[0], from_alignms=gld_alignms, to_alignms=prd_alignms)

    if len([span for span in prd_alignms if len(prd_alignms[span]) > 1]) == 0:
        prd_aligned = True
    else:
        prd_aligned = False
        prd_alignms, gld_alignms = perform_op(op=op[0], from_alignms=prd_alignms, to_alignms=gld_alignms)

    if (gld_aligned and prd_aligned) or len(op) == 1:
        return gld_alignms
    else:
        op.pop(0)
...
def calc_fscore(pv, rv, beta: int = 1):
    """Python function for calculating recall
    ...
    """
    try:
        f = (1 + (beta * beta)) * ((pv * rv) / ((beta * beta * pv) + rv))
    except:
        f = 0
    return f
